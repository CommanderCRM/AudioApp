...
- Установить пакет ```sqlite3```. Перейти в ```audioserver/tests```. Отредактировать функции из ```logic.py``` и их вызов таким образом, чтобы можно было передать внешний параметр ```engine```.
В функциях ```test_logic.py``` передавать тестовый ```engine```. Далее выполнить ```pytest -rA .```
